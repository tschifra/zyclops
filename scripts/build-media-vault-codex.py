...
THEMES = json.loads((VAULT / 'meta/prompts.json').read_text())
...
def poster_video(theme, path):
    image = VAULT / 'images' / f'zyclops-{theme}-v1.jpg'
    effect = "scale=2160:2160,zoompan=z='1.005+0.015*(1-cos(2*PI*on/180))/2':x='iw/2-iw/zoom/2':y='ih/2-ih/zoom/2':d=180:s=720x720:fps=30"
...
def cine_video(name, theme, path):
    source=np.asarray(Image.open(VAULT/'images'/f'zyclops-{theme}-v1.jpg').convert('RGB').resize((SIZE,SIZE),Image.Resampling.LANCZOS))
...
VIDEOS = [
    ...
    ('hodl-motion','hodl','motion_poster','HODL · moving poster'),
...
def package():
    ...
    for t in THEMES:
        ...
        assert (VAULT/path).exists(),f'Missing image: {path}'
        assert (VAULT/path).stat().st_size <= 5_000_000,path
        assets.append(common(t,path,t['id'],'image',thumb))
    for ident,theme,kind,title in VIDEOS:
        path=f'video/zyclops-{ident}-v1.mp4'; thumb=f'thumbs/zyclops-{ident}-v1.jpg'
        info=probe(VAULT/path); stream=next(s for s in info['streams'] if s['codec_type']=='video')
        assert stream['codec_name']=='h264' and stream['pix_fmt']=='yuv420p',path
        assert stream['width']==720 and stream['height']==720,path
        assert stream['r_frame_rate']=='30/1',path
        assert abs(float(info['format']['duration'])-6)<.05,path
        a=common(BY_ID[theme],path,ident,'video',thumb)
        a.update(title=title,duration_seconds=6,fps=30,animation_type=kind,poster_path=thumb,poster_url=BASE+thumb,has_audio=False)
        if kind=='character_loop': a['creative_id']=f'zyclops-{ident}-v1'
        ...
        else: a['alt_text']={'gm-wave':'A black-and-gold ZYCLOPS sticker waves hello above the letters GM.','pump-fists':'ZYCLOPS raises his fists as his golden eye and surrounding sparkles pulse.','fine-coffee':'ZYCLOPS calmly drinks coffee as cartoon flames flicker and steam drifts upward.'}[ident]
        gif=f'gif/zyclops-{ident}-v1.gif'
        if (VAULT/gif).exists():
            f=VAULT/gif; assert f.stat().st_size<=5_000_000,gif
            with Image.open(f) as im: assert im.n_frames<=350 and im.size==(480,480),gif
            a['alternates']=[{'format':'gif','mime_type':'image/gif','path':gif,'url':BASE+gif,'width':480,'height':480,'fps':12,'bytes':f.stat().st_size,'sha256':hashlib.sha256(f.read_bytes()).hexdigest()}]
        assets.append(a)
    manifest={'schema_version':'1.0','library_version':'2026-09-06.1','brand':'ZYCLOPS','ticker':'$ZYCL','base_url':BASE,'access':'Unlisted, publicly accessible by URL; not authenticated.','counts':{'images':24,'videos':12,'gif_alternates':7},'assets':assets}
    assert len({a['id'] for a in assets})==len(assets)
    (VAULT/'manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n')
    fields=['id','creative_id','title','collection','media_type','url','mime_type','width','height','bytes','duration_seconds','animation_type','caption','alt_text','tags']
    ...
    bundle=VAULT/'zyclops-media-pack-v1.zip'
    with zipfile.ZipFile(bundle,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=6) as archive:
        for file in sorted(VAULT.rglob('*')):
            if file.is_file() and file != bundle and not file.name.startswith('.'):
                archive.write(file,'media-vault/'+str(file.relative_to(VAULT)))
    print(json.dumps({'assets':len(assets),'zip_bytes':bundle.stat().st_size,'largest_image':max(a['bytes'] for a in assets if a['media_type']=='image'),'largest_video':max(a['bytes'] for a in assets if a['media_type']=='video')},indent=2))
