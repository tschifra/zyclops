# ZYCLOPS sticker pack

| File | Emoji | Title |
|---|---|---|
| `01-scowl.png` | 😐 | The stare |
| `02-gm.png` | 👋 | GM |
| `03-diamond.png` | 💎 | Diamond hands |
| `04-wagmi.png` | 👍 | WAGMI |
| `05-ser.png` | 👉 | Ser |
| `06-copium.png` | 😭 | Copium |
| `07-fine.png` | 🔥 | This is fine |
| `08-shh.png` | 🤫 | Shielded |
| `09-laser.png` | 😤 | Laser eye |
| `10-ngmi.png` | 🤦 | NGMI |
| `11-rekt.png` | 😵 | Rekt |
| `12-blink.png` | 😉 | The blink |
| `13-peek.png` | 🫣 | Redacted |
| `14-based.png` | 😍 | Based |
| `15-moon.png` | 🌙 | Wen moon |
| `16-hodl.png` | 🗿 | HODL |

Official pack short name: ZYCLOPSzec
Intended link: https://t.me/addstickers/ZYCLOPSzec
That link 404s in Telegram until the pack is published.

Publish via @Stickers → /newpack → title ZYCLOPS → upload these PNGs
with the emoji above → /publish → short name ZYCLOPSzec exactly.
