# ZYCLOPS media vault · volume 6

44 still images and 8 silent looping scenes. Still artwork is 1200 × 1200 JPEG. Loops are closed palindromes: the first frame matches the last, there is no audio, and they are shipped as GIF (exact loop, good for X) with a smaller WebP alternate.
...
- Download: https://zyclops.xyz/media-vault/zyclops-media-pack-v6.zip

The gallery is unlisted and excluded from indexing, but anyone with its URL can access it. Share it with holders to download pictures, copy captions, and grab a loop. Older ZIP links (`v1`–`v5`) serve the current pack.

## Bot integration

Fetch the current manifest before selecting artwork. `schema_version` remains `1.0`; `library_version` is `2026-09-06.6`. The `assets` array is the current posting inventory. `retired_asset_ids` lists older Ken Burns / sticker loops that stay retired.

Each retained still keeps its existing `id`, `creative_id`, `path`, `url` and SHA-256. Volume 6 adds two still memes: The chart and Current assignment (Find Zatoshi). Filter `release: 6` for these additions, or `release: 5` for the preceding 16-image batch. Existing artwork and loops remain available. Selection fields include `collection` (`meme` or `artwork`), `tags`, `captions`, `alt_text`, and `suggested_cooldown_hours`. Use `creative_id` to pair a still with its loop and to avoid repeats.
